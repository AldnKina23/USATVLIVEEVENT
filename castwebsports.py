...
OUTPUT_DIR = Path("UsaLiveEvent")
OUTPUT_FILE = OUTPUT_DIR / "CastwebSports.m3u8"
TIVIMATE_OUTPUT_FILE = OUTPUT_DIR / "CastwebSports_TiviMate.m3u8"
...
def write_playlist_tivimate(streams: List[Dict], filepath: Path):
    ...
    print(f"TiviMate playlist saved: {filepath}")
# --------------------------------------------------------------------------------
    ...
    try:
        async with session.get(url, timeout=10, allow_redirects=True, headers=request_headers) as response:
            if response.status == 200:
                print(f"URL Verified (200 OK): {url}")
                return True
            else:
    ...
    try:
        ...
        for i in range(count):
            ...
            for stream_url in reversed(new_urls):
                if await verify_stream_url(session, stream_url, headers=verification_headers):
                    print(f"Found valid stream after clicking '{link_text}'.")
                    return stream_url
...
def write_playlist(streams: List[Dict], filepath: Path):
    ...
    print(f"Playlist with {len(streams)} streams saved successfully to {filepath}!")

async def main():
    print("Starting Sports Webcast Updater...")
    ...
    # Write VLC Format ke folder UsaLiveEvent
    write_playlist(all_streams, OUTPUT_FILE)

    # Write TiviMate Format ke folder UsaLiveEvent
