...
OUTPUT_FILE = OUTPUT_DIR / "centerstrm.m3u"
...
def write_playlist_file(content: str):
    """Ensure directory exists and write content to playlist file"""
    try:
        OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        OUTPUT_FILE.write_text(content, encoding="utf-8")
        log.info(f"Successfully wrote entries to {OUTPUT_FILE}")
...
async def scrape() -> None:
    ...
    if not events:
        write_playlist_file(build_playlist(cached))
        return
