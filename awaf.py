...
# Simpan file di dalam folder UsaLiveEvent
OUTPUT_DIR = Path("UsaLiveEvent")
OUTPUT_FILE = OUTPUT_DIR / "awaf.m3u"
...
async def scrape() -> None:
    ...
    # Pastikan direktori UsaLiveEvent ada sebelum menyimpan file
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    # Write playlist
    out = build_playlist(urls)
    OUTPUT_FILE.write_text(out, encoding="utf-8")
    log.info(f"Successfully wrote {len(urls)} entries to {OUTPUT_FILE}")
